...
<f:form method="post" modelAttribute="hoaDon">
    <div>
    ...
    <div>
        ...
        <f:select path="nhanVienId">
            <f:option value="${options.nhanVienId}">${options.maNV}</f:option>
    ...
    <div>
        ...
        <f:select path="khachHangId">
            <f:option value="${options.khachHangId}">${options.maKH}</f:option>
            <f:options items="${khachHangList}" itemValue="id" itemLabel="maKH"></f:options>
        </f:select><br>
        <f:errors path="nhanVienId" cssStyle="color: red"/>
    </div>
    <input formaction="/hoa-don/update/${hoaDon.id}" type="submit" value="Submit">
</f:form>
