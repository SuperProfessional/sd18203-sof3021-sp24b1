...
public class HoaDonController {
    ...
    public String store(
            Model model,
            @Valid @ModelAttribute(name = "hoaDon") HoaDonDto dto,
            BindingResult result
    ) {

        if (result.hasErrors()) {
            model.addAttribute("hoaDon",dto);
            return "admin/ql-hoa-don/";
        }
    ...
    public String update(
        ...
        boolean isExistMaNV =
                this.nhanVienDtoList.stream()
                        .anyMatch(
                                nhanVien -> Objects.equals(dto.getNhanVienId(), id)
                        );

        boolean isExistMaKH =
                this.khachHangDtoList.stream()
                        .anyMatch(
                                khachHang -> Objects.equals(dto.getKhachHangId(), id)
                        );

        if (!(isExistMaNV && isExistMaKH)) {
            return "redirect:/hoa-don/edit/{" + dto.getId() + "}";
        }
